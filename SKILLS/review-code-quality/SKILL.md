...
description: Use when dispatched as a start-action code reviewer subagent to review one implemented scope for correctness, regressions, and verification quality while still surfacing proven plan drift instead of ignoring it.
---
...
Code review is still spec-aware. If a broken plan step is proven from repo reality, surface it as `PLAN_DRIFT_EVIDENCE`.

## Review Order

1. Re-read the immutable spec for the scope.
2. Re-read the frozen plan for the scope.
3. Re-read the implementation diff, tests, and any must-read files.
4. Evaluate:
   ...
   - whether the frozen plan still matches executable reality
5. Return one status.
