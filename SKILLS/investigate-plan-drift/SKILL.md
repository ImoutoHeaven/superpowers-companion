...
description: Use when dispatched as a start-action read-only investigation subagent to compare immutable spec, frozen plan, and actual codebase evidence and return a concrete judgment about plan drift and safe continuation.
---
...
Your job is not to implement, rewrite docs, or decide final workflow policy. Your job is to compare immutable spec, frozen plan, and actual repo or worktree evidence, then return a concrete judgment that P9 can adjudicate with.
...
3. The frozen plan is derived coordination only.
...
3. If not, does continuing require immutable spec change or doc reconvergence?
...
- `PLAN_DRIFT_REQUIRES_RECONVERGENCE`
...
- `PLAN_DRIFT_REQUIRES_RECONVERGENCE` -> `SAFE_TO_CONTINUE: NO`
...
For `PLAN_DRIFT_REQUIRES_RECONVERGENCE`, state whether the blocker is lack of any safe route or a required immutable spec change.

Use `INSUFFICIENT_EVIDENCE` only when the available evidence is genuinely not enough to establish whether a safe executable route exists.
...
- execution may continue under that adjudicated understanding while doc reconvergence happens later

Use `PLAN_DRIFT_REQUIRES_RECONVERGENCE` only when there is no safe executable route yet, or when continuing would require immutable spec change before further execution.
...
- require the immutable spec, frozen plan, relevant repo or worktree evidence, and the actual drift claims or payloads before issuing a judgment
