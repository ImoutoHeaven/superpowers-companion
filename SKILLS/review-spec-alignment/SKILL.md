...
description: Use when dispatched as a start-action spec reviewer subagent to review one implemented scope against the immutable spec first, and to detect when the frozen plan has become a stale or conflicting derived artifact.
---
...
Then decide whether the frozen plan remains a valid guide. If it does not, return `PLAN_DRIFT_EVIDENCE` with evidence.

## Review Order

1. Re-read the immutable spec for the current scope.
2. Re-read the frozen plan for the current scope.
3. Re-read the implementation diff and any must-read repo files.
4. Decide which case you are in:
   - Required review artifacts are missing or only partially handed off: `NEEDS_CONTEXT`.
   - The implementation is spec-correct and the plan remains a valid derived guide: `PASS`.
   - The implementation misses spec requirements, adds forbidden behavior, or is internally inconsistent: `CHANGES_REQUIRED`.
   - The implementation is spec-correct but the plan step is stale, unexecutable, or would require spec drift if enforced literally: `PLAN_DRIFT_EVIDENCE`.
...
- if the immutable spec, frozen plan, exact scope, or implementation diff is missing, return `NEEDS_CONTEXT`
