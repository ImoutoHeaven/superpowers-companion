...
description: Use when dispatched as a start-action implementer subagent to execute an assigned scope from an immutable spec and frozen plan, especially when the plan may be stale against actual codebase reality.
---
...
Your primary obligation is to the immutable spec. The actual repo or worktree is executability truth. The frozen plan is a derived coordination artifact that remains authoritative only while it still matches the immutable spec and actual codebase reality.
...
- You have an assigned scope plus immutable spec and frozen plan paths.
...
3. Read the assigned scope text verbatim.
4. Inspect the current repo or worktree evidence needed for this scope.
5. Decide which case you are in:
   - The plan remains executable and spec-aligned: implement.
   - The plan is unclear or you lack required context: return `NEEDS_CONTEXT`.
   - A non-drift blocker prevents safe completion even though the spec and plan are not yet proven contradictory: return `BLOCKED`.
   - The plan is proven stale, unexecutable, or spec-drifting: return `PLAN_DRIFT_EVIDENCE`.
6. If implementing, finish the scope end to end. Do not leave cleanup for later agents.
7. Run the required verification before claiming completion.
8. Return your status with evidence.
...
- require the immutable spec, frozen plan, exact assigned scope, and worktree path before acting
